    ...
    if (isWorldReady[0] && isWorldReady[1]) {
      ...
      for (var i = 0; i < camera.Rayos.length; i++) {
        ...
        if (colisionesRe.length > 0) {
          // SI la distancia del objeto mas cercano a mi es menor a 1 Y su propiedad parent (que yo la agregue al momento de crear el objeto)
          //  (se puede ver en la funcion donde creo cada engrane)
          // si su esta es atrapada = false , significa que aun no la tomo y puedo sumar puntos.
          if (
            colisionesRe[0].distance < 1 &&
            colisionesRe[0].object.parent.atrapada == false
          ) {
            // Recorro todo el arreglo de mis objetos coleccionables
            for (var i = 0; i < objetosColeccionables.length; i++) {
              // checho si de mi arreglo original la propiedad name es igual al name del modelo mas cercano a mi
              //  si es asi, le cambio el estado de atrapada a ese arreglo, para que luego ya no sume puntos
              if (
        ...
        if (colisionesRe.length > 0) {
          if (
            ...
            if (reproducirParticulas == false) {
              ...
              spawnParticulas(colisionesRe[0].object.parent.position)
              scene.remove(colisionesRe[0].object.parent)
              totalesRecoleccion = int.parse(totalesRecoleccion) + 1
              localStorage.setItem('totalesRecoleccion', totalesRecoleccion)
              referencia.update({
                PuntuacionEquipo: totalesRecoleccion,
              })
